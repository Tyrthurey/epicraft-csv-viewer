...
import {fetchModrinthProjectVersions, fetchModrinthProjects} from '@/lib/modrinth';
import {fetchCurseForgeMods} from '@/lib/curseforge';

export async function GET(request: NextRequest) {
    const {searchParams} = new URL(request.url);
    const modrinthIds = searchParams.get('modrinthIds')?.split(',').filter(id => !!id) || [];
    const curseforgeIds = searchParams.get('curseforgeIds')?.split(',').filter(id => !!id).map(id => parseInt(id)).filter(id => !isNaN(id)) || [];
    ...
    if (modrinthIds.length > 0) {
        // 1. Fetch all project versions in parallel
        const allProjectVersions = await Promise.all(
            modrinthIds.map(async id => {
                try {
                    return {id, versions: await fetchModrinthProjectVersions(id)};
                } catch (e) {
        ...
        allProjectVersions.forEach(({versions}) => {
            versions.forEach(v => {
                ...
                const isRelevant = targets.some(t =>
    ...
    if (curseforgeIds.length > 0) {
        ...
        for (const mod of mods) {
            for (const target of targets) {
                ...
                const matchingFiles = mod.latestFiles.filter(f =>
                    f.gameVersions.includes(target.version) &&
                    (mod.latestFilesIndexes.some(idx =>
                        idx.gameVersion === target.version &&
                        (idx.modLoader === targetLoaderId || idx.modLoader === 0)
                    )) &&
                    f.gameVersions.some(gv => {
                        if (target.loader === 'fabric') return gv.toLowerCase() === 'fabric';
                        if (target.loader === 'forge') return gv.toLowerCase() === 'forge';
                        if (target.loader === 'neoforge') return gv.toLowerCase() === 'neoforge';
                        return false;
                    })
                );
                ...
                if (!file) {
                    const idx = mod.latestFilesIndexes.find(i =>
            ...
            for (const target of targets) {
                ...
                if (file) {
                    file.dependencies.forEach(d => {
                        ...
                        if (d.relationType === 3 || d.relationType === 1 || d.relationType === 6) {
                            targetDeps.required.push(depData);
                        } else {
                            targetDeps.optional.push(depData);
                        }
